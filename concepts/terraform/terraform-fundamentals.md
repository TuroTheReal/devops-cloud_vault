...
### ❌ Bad for
- **Server configuration** → Use [[concepts/ansible/ansible-fundamentals]] instead
- **One-shot manual setup** → AWS Console faster for quick tests

---

## 📚 Key Concepts

### 1. The Workflow (init → plan → apply → destroy)
...
**Why important**:
- No tfstate → Terraform will recreate everything (duplicates!)
- 2 people with different tfstate → chaos
...
**What I did wrong**:
```hcl
# ❌ Wrong - Launch Ansible immediately after EC2 creation
...
**Solution**:
```hcl
# ✅ Correct - Wait for SSH to be actually ready
