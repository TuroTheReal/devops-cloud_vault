...
docker network disconnect backend myapp
...
docker exec frontend dig api

# Check container networks
docker inspect myapp --format='{{range .NetworkSettings.Networks}}{{.NetworkID}} {{end}}'

# Debug Traefik network selection
...
```
Service needs:          Put on network:
─────────────────────────────────────────
Only DB access     →    private only
Only internet      →    public only
Both DB + internet →    public + private
```

### 2. Defense in Depth
```
Layer 1: Firewall (host)
Layer 2: Network isolation (Docker)
Layer 3: Service authentication (app)
Layer 4: Encryption (TLS)

If Layer 1 fails, Layer 2 still protects DB
```

### 3. Zero-Trust Checklist
- [ ] Database on private network only
- [ ] `internal: true` for DB network
- [ ] No published DB ports
- [ ] Frontend/API bridge networks (not on DB network unless needed)
- [ ] Traefik on public network only
- [ ] Monitor network connections (unexpected communication?)
...
```
From Glasck (2025-12): Implemented zero-trust network architecture
2025-12-17: Designed public/private network separation
2025-12-18: Fixed multi-network Traefik routing (2h debugging)
...
| **TOTAL** | **1h (12.5%)** | **7h (87.5%)** | **8h** |

**Ratio**: 12.5% ✅ (target <30%)
...
- Project: Glasck production deployment
- Source: [Glasck/deployment/docker/stack.vps.yml](/home/artberna/abGitHub/Glasck/deployment/docker/stack.vps.yml)
...
### Understanding
- [x] Explain zero-trust network architecture
- [x] Design public/private network separation
- [x] Identify which services need which networks
- [x] Debug multi-network routing issues

### Application
- [x] Production network isolation (Glasck)
- [x] Internal network flag usage
- [x] Multi-network service configuration
- [x] Traefik network specification
